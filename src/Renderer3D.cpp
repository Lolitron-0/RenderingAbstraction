...
Scope<Ra::Renderer3D::SceneData> Renderer3D::s_SceneData =
    std::make_unique<Renderer3D::SceneData>();
...
void Renderer3D::Init()
{
    ...
    Texture::NullTexture = Texture::Create((uint8_t*)nullTexData, 1, 1, 4);

    Storage.BlinnPhongShader =
        Shader::Create(GetShadersDir() + "blinn-phong.vert",
                       GetShadersDir() + "blinn-phong.frag");
    Storage.SkyboxShader = Shader::Create(GetShadersDir() + "skybox.vert",
                                          GetShadersDir() + "skybox.frag");
    Storage.DepthShader = Shader::Create(GetShadersDir() + "depth.vert",
                                         GetShadersDir() + "depth.frag");
    ...
    postprocessingBufferProps.Attachments = { TextureFormat::ColorLinear,
                                              Ra::TextureFormat::DepthStencil };
    s_RendererData.PostprocessingBuffer =
        Framebuffer::Create(postprocessingBufferProps);
}
...
void Renderer3D::DrawSkybox(const Skybox& skybox)
{
    ...
    Storage.SkyboxShader->SetMat4(
        "u_ViewProjection", s_SceneData->ProjMatrix *
                                glm::mat4(glm::mat3(s_SceneData->ViewMatrix)));
    skybox.Bind();
...
{
    s_RendererData.PostprocessingBuffer->Resize(
        static_cast<std::uint32_t>(size.x), static_cast<std::uint32_t>(size.y));
}

glm::vec2 Renderer3D::GetViewportSize()
{
    return { s_RendererData.PostprocessingBuffer->GetProperties().Width,
             s_RendererData.PostprocessingBuffer->GetProperties().Height };
}
...
void Renderer3D::BeginScene(const glm::mat4& viewMatrix,
...
void Renderer3D::EndScene()
{
    ...
    s_SceneData->ViewProjectionMatrix =
        s_SceneData->ProjMatrix * s_SceneData->ViewMatrix;
...
void Renderer3D::Submit(
    const Ref<VertexArray>& vertexArray, const Transform& transform,
    Ref<Shader>& shader,
    RendererAPI::DrawMode mode /*= RendererAPI::DrawMode::Triangles*/)
{
    ...
    RA_ASSERT(s_SceneData->SubmittedPointLights <= 128,
...
void Renderer3D::Submit(
    const Ref<Mesh>& mesh, const Transform& transform,
    RendererAPI::DrawMode mode /*= RendererAPI::DrawMode::Triangles*/)
{
    PROFILER_SCOPE("Renderer::Submit( Mesh )  -  " + mesh->GetPath());
    s_SceneData->RenderQueue.push_back(
        [mesh, transform, mode](Ref<Shader> shader)
        {
...
void Renderer3D::DrawVector(const glm::vec3& position,
...
{
    std::string uniformLightToken =
        "u_PointLights[" + std::to_string(s_SceneData->SubmittedPointLights++) +
        "].";
    ...
    Storage.BlinnPhongShader->SetFloat(uniformLightToken + "Intensity",
                                       light.Intensity);
}

void Renderer3D::SubmitLight(const DirLight& light)
{
    RA_ASSERT(s_SceneData->SubmittedDirLights == 0,
              "Multiple dir lights not supported!")
    std::string uniformLightToken = "u_DirLight.";
    Storage.BlinnPhongShader->Bind();
    Storage.BlinnPhongShader->SetVec3(uniformLightToken + "Direction",
                                      light.Direction);
    Storage.BlinnPhongShader->SetVec3(uniformLightToken + "Color", light.Color);
    Storage.BlinnPhongShader->SetFloat(uniformLightToken + "Intensity",
                                       light.Intensity);
    ...
    if (s_SceneData->DirLightObject.Direction != light.Direction)
    {
        ...
        auto lightSpaceView = glm::lookAt(
            ...
            s_SceneData->DirLightObject.Direction, { 0.f, 1.f, 0.f });
        auto lightSpaceProj =
            glm::ortho(-200.f, 200.f, -200.f, 200.f, 0.1f, 1000.f);
