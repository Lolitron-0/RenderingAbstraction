...
void AttachColorTexture(RendererId texture, GLenum internalFormat,
                        GLenum format, std::uint32_t width,
                        std::uint32_t height, int samples, std::size_t index)
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + (GLint)index,
                           TextureTarget(multisampled), texture, 0);
}

void AttachDepthTexture(RendererId texture, GLenum format,
                        GLenum attachmentType, std::uint32_t width,
                        std::uint32_t height, int samples)
{
    bool multisampled = samples > 1;
    BindTexture(texture, multisampled);
...
void OpenGLFramebuffer::StartWriting()
{
    ...
    glClear((m_ColorAttachments.size() ? GL_COLOR_BUFFER_BIT : 0) |
            GL_DEPTH_BUFFER_BIT);
}
...
RendererId
OpenGLFramebuffer::GetDrawTextureHandle(std::size_t index /*= 0*/) const
{
    return m_Properties.Samples > 1 ? m_ResolvedAttachments[index]
...
int OpenGLFramebuffer::ReadPixel(std::uint32_t attachmentIndex, int x, int y,
                                 bool leaveBound)
{
    int pixelData;
    RA_ASSERT(attachmentIndex < m_ColorAttachments.size(),
...
{
    RA_ASSERT(m_Properties.Samples == 1,
              "Depth texture binding for multisampled buffers not supported!");
...
{
    glActiveTexture(GL_TEXTURE0 + Renderer3D::GetLastTextureUnit() + 1);
    Utils::BindTexture(GetColorAttachmentHandle(index),
...
void OpenGLFramebuffer::Invalidate_()
{
    ...
    for (std::size_t i = 0; i < m_ColorAttachmentSpecifications.size(); i++)
    {
        ...
        case TextureFormat::RGBA8:
            Utils::AttachColorTexture(textureId, GL_RGBA8, GL_RGBA,
                                      m_Properties.Width, m_Properties.Height,
                                      m_Properties.Samples, i);
            break;
        case TextureFormat::R32:
            Utils::AttachColorTexture(textureId, GL_R32I, GL_RED_INTEGER,
                                      m_Properties.Width, m_Properties.Height,
                                      m_Properties.Samples, i);
            break;
        default:
    ...
    case TextureFormat::Depth24Stencil8:
        Utils::AttachDepthTexture(
            m_DepthAttachment, GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL_ATTACHMENT,
            m_Properties.Width, m_Properties.Height, m_Properties.Samples);
        break;
    case TextureFormat::DepthComponent:
        Utils::AttachDepthTexture(m_DepthAttachment, GL_DEPTH_COMPONENT,
                                  GL_DEPTH_ATTACHMENT, m_Properties.Width,
                                  m_Properties.Height, m_Properties.Samples);
        break;
    }

    auto a = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    RA_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) ==
    ...
    {
        RA_ASSERT(m_ColorAttachments.size() <= 4,
                  "Framebuffers support up to 4 color attachments!");
...
void OpenGLFramebuffer::Cleanup_()
{
    ...
    if (m_ResolveFramebufferHandle)
    {
        ...
        glDeleteTextures((GLsizei)m_ResolvedAttachments.size(),
                         m_ResolvedAttachments.data());
    }
}
...
void OpenGLFramebuffer::Resolve_()
{
    ...
    glBlitFramebuffer(0, 0, m_Properties.Width, m_Properties.Height, 0, 0,
                      m_Properties.Width, m_Properties.Height,
