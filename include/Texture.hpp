...
public:
    ...
    static Ref<Texture>
    Create(std::uint8_t* rawData, std::uint32_t width, std::uint32_t height,
...
protected:
    virtual void
    LoadFromFile_(const std::string& path,
                  TextureFormat format = TextureFormat::ColorLinear,
                  TextureType type = TextureType::Diffuse) = 0;
    virtual void
    LoadFromData_(std::uint8_t* rawData, std::uint32_t width,
