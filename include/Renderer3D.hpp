...
public:
    ...
    // static void Submit(const std::function<void(void)>& onEnd, const F& call,
    // Args&&... args)
    //{
     ...
     */
    static void
    Submit(const Ref<VertexArray>& vertexArray, const Transform& transform,
           Ref<Shader>& shader,
           RendererAPI::DrawMode mode = RendererAPI::DrawMode::Triangles);

    static void
    Submit(const Ref<Mesh>& mesh, const Transform& transform,
           RendererAPI::DrawMode mode = RendererAPI::DrawMode::Triangles);

    static void DrawVector(const glm::vec3& position,
    ...
    static auto GetLastTextureUnit() -> std::uint8_t;
