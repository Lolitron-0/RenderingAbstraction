 ...
 * Objects of this class is used to set up a proper rendering context (load
 * correct API version, etc.) and is responsible for window <-> renderer
 * interaction (such as buffer swapping)
 */
class RenderingContext
{
public:
    /**
     * @brief Struct with all rendering API info (for client logging or
     * whatever)
     */
    struct APIinfo
    {
        ...
        std::string Version;
        std::any
            NativeInfo; /*!< Contains native info object for API (e.g.
                           gladGLversionStruct GLVersion for OpenGL (glad)) */
    };
    ...
    /**
     * @brief Factory method for proper context creation (OpenGL context, Vulkan
     * context, etc.)
     * @param windowHandle Native handle of the window to set the current
     * context (e.g. GLFWwindow* for OpenGL)
